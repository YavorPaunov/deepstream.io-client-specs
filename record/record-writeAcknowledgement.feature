...
Scenario: Record write acknowledgement
	...
	When the server sends the message R|A|S|happyRecord+
	...
	When the server sends the message R|WA|happyRecord|[101]|L+
	...
	When the server sends the message R|WA|happyRecord|[102]|L+
	Then the client is notified that the record "happyRecord" was written without error
	...
	When the server sends the message R|WA|happyRecord|[103]|SError writing record to storage+
	...
	When the server sends the message R|WA|happyRecord|[104]|SError writing record to cache+
	...
	When the server sends the message R|E|VERSION_EXISTS|happyRecord|105|{"validData":"newErrorData"}|{"writeSuccess":true}+
	Then the last message the server recieved is R|P|happyRecord|105|validData|SsomeData|{"writeSuccess":true}+
	When the server sends the message R|WA|happyRecord|[106]|L+
	Then the client is notified that the record "happyRecord" was written without error
